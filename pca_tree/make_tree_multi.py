...
def draw_lines(W, data_x, data_y, pdf):
    ...
    ax.scatter3D(data_x[0, positive_indices], data_x[1, positive_indices], data_x[1, positive_indices], c='blue', s=1)
    ax.scatter3D(data_x[0, negative_indices], data_x[1, negative_indices], data_x[1, negative_indices], c='yellow', s=1)
    grain = 0.001
    x_min, x_max = data_x[0, :].min() - 1, data_x[0, :].max() + 1
    y_min, y_max = data_x[1, :].min() - 1, data_x[1, :].max() + 1
    z_min, z_max = data_x[1, :].min() - 1, data_x[1, :].max() + 1
    x1 = np.arange(x_min, x_max, grain)
    y1 = np.arange(y_min, y_max, grain)
    x1, y1 = np.meshgrid(x1, y1)
    for w in W:
        if w[2] == 0:
            z1 = -1 * (w[3] + w[0] * x1 + w[1] * y1) / 0.000000000000000000001
        else:
            z1 = -1 * (w[3] + w[0] * x1 + w[1] * y1) / w[2]
...
def draw_points(data_x, data_y, labels, pdf):
    ...
    fig = plt.figure()
    ax = plt.axes(projection='3d')
    ax.scatter3D(data_x[0, true_positive_indices], data_x[1, true_positive_indices], data_x[2, true_positive_indices], c='blue', s=1)
    ax.scatter3D(data_x[0, true_negative_indices], data_x[1, true_negative_indices], data_x[2, true_negative_indices], c='yellow', s=1)
    ax.scatter3D(data_x[0, false_positive_indices], data_x[1, false_positive_indices], data_x[2, false_positive_indices], c='lime', s=1)
    ax.scatter3D(data_x[0, false_negative_indices], data_x[1, false_negative_indices], data_x[2, false_negative_indices], c='pink', s=1)

    x_min, x_max = data_x[0, :].min() - 1, data_x[0, :].max() + 1
    y_min, y_max = data_x[1, :].min() - 1, data_x[1, :].max() + 1
    z_min, z_max = data_x[2, :].min() - 1, data_x[2, :].max() + 1
    ax.axes.set_xlim3d(left=x_min, right=x_max)
    ax.axes.set_ylim3d(bottom=y_min, top=y_max)
    ax.axes.set_zlim3d(bottom=z_min, top=z_max)
    ax.legend(loc='upper right')
...
def draw_set_separator(w_init, data_x, data_y, candidate_points_pos, candidate_points_neg, iter, pdf):
    fig = plt.figure()
    ax = plt.axes(projection='3d')
    ax.scatter3D(data_x[0], data_x[1], data_x[2], c='red', s=5)
    ax.scatter3D(data_y[0, :], data_y[1, :], data_y[2, :], c='blue', s=5)
    ax.scatter3D(candidate_points_pos[0, :], candidate_points_pos[1, :], candidate_points_pos[2, :], c='lime', s=3)
    ax.scatter3D(candidate_points_neg[0, :], candidate_points_neg[1, :], candidate_points_pos[2, :], c='pink', s=3)
    grain = 0.001
    x_min, x_max = -2, 2
    y_min, y_max = -2, 2
    z_min, z_max = -2, 2
    ...
    ax.axes.set_ylim3d(bottom=y_min, top=y_max)
    ax.axes.set_zlim3d(bottom=z_min, top=z_max)
    fig.savefig('separator_hyp' + str(w_init[0]) + str(iter) + '.png')
...
def great_pick_algo(candidate_points_pos, candidate_points_neg, p, pdf):
    ...
    master_midpoint_proj[0:-1] = master_midpoint[0:-1]-(np.dot(w, master_midpoint)/np.dot(w[0:-1], w[0:-1]))*w[0:-1]
...
def main():
    for a in range(1, 2):
        np.random.seed(a)
        data = pd.read_csv('../union_of_convex_datasets/three_3D_hyperplanes.csv', header=None).values
        ...
        pdf.output('pca_tree_multi' + str(a) + '.pdf')
